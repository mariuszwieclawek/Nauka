...
//	expander_looptest(?expander_address?);
	
	// ToDo 2.4 Turn on LCD1602 backlight - 4th bit on I/O expander.

//	I2C_XXXXX(?expander_address?, ?data?);
...
//	I2C_ReadReg (?mma_address?, ?who_am_i?, &readout);
...
//	I2C_WriteReg(?mma_address?, ?ctrl_reg1?, ?set_active_bit?);	 		// Set ACTIVE bit in CTRL_REG1 in MMA8451

//	I2C_ReadReg (?mma_address?, ?out_z_msb?, &readout); // Get OUT_Z_MSB from MMA8451 (MSB 8 bits from 14 in total)
...
//	I2C_ReadRegBlock(?mma_address?, ?out_x_msb?, ?block_size?, arrayXYZ);
