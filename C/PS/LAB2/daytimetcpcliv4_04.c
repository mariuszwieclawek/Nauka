...
#define MSS 1300
	...
	if( gettimeofday(&stop,0) != 0 ){
		...
		if(time_start == 1){
			s=(int)(stop.tv_sec-start.tv_sec);
			double tr = (size*8.0)/(s*1000000.0);
		    fprintf(stderr,"Reading from socket in: %d s, %d us : in %d segments (%lu)(%lf mbit/s)\n", s,
		      (int)(stop.tv_usec-start.tv_usec), count, size, tr );
		}
	  //}
	}
